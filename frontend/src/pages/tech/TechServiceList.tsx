import { useState } from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPlus, faEdit, faTrash, faTimes } from '@fortawesome/free-solid-svg-icons';

interface TechService {
  id: string;
  name: string;
  version: string;
  status: 'running' | 'stopped' | 'error';
  description: string;
}

interface ModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Omit<TechService, 'id'>) => void;
  editingService?: TechService | null;
}

function ServiceModal({ isOpen, onClose, onSave, editingService }: ModalProps) {
  const [formData, setFormData] = useState<Omit<TechService, 'id'>>({
    name: editingService?.name || '',
    version: editingService?.version || '',
    status: editingService?.status || 'running',
    description: editingService?.description || '',
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave(formData);
    onClose();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800">
            {editingService ? '编辑技术服务' : '新增技术服务'}
          </h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors">
            <FontAwesomeIcon icon={faTimes} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">服务名称</label>
            <input
              type="text"
              value={formData.name}
              onChange={e => setFormData({ ...formData, name: e.target.value })}
              required
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
              placeholder="输入服务名称"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">版本号</label>
            <input
              type="text"
              value={formData.version}
              onChange={e => setFormData({ ...formData, version: e.target.value })}
              required
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
              placeholder="如：v1.0.0"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">状态</label>
            <select
              value={formData.status}
              onChange={e => setFormData({ ...formData, status: e.target.value as TechService['status'] })}
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
            >
              <option value="running">运行中</option>
              <option value="stopped">已停止</option>
              <option value="error">异常</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">描述</label>
            <textarea
              value={formData.description}
              onChange={e => setFormData({ ...formData, description: e.target.value })}
              rows={3}
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all resize-none"
              placeholder="简要描述服务功能"
            />
          </div>
          <div className="flex items-center gap-3 pt-2">
            <button
              type="submit"
              className="px-5 py-2.5 bg-blue-500 text-white font-medium rounded-xl hover:bg-blue-600 transition-all"
            >
              保存
            </button>
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 bg-slate-100 text-slate-600 font-medium rounded-xl hover:bg-slate-200 transition-all"
            >
              取消
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}

const initialServices: TechService[] = [
  { id: '1', name: '数据清洗服务', version: 'v2.1.0', status: 'running', description: '提供数据清洗和标准化处理' },
  { id: '2', name: '数据同步服务', version: 'v1.5.2', status: 'running', description: '支持多源数据实时同步' },
  { id: '3', name: '数据分析服务', version: 'v3.0.1', status: 'error', description: '提供数据分析报表和可视化' },
];

export default function TechServiceList() {
  const [services, setServices] = useState<TechService[]>(initialServices);
  const [showModal, setShowModal] = useState(false);
  const [editingService, setEditingService] = useState<TechService | null>(null);

  const handleAdd = () => {
    setEditingService(null);
    setShowModal(true);
  };

  const handleEdit = (service: TechService) => {
    setEditingService(service);
    setShowModal(true);
  };

  const handleSave = (data: Omit<TechService, 'id'>) => {
    if (editingService) {
      setServices(services.map(s => s.id === editingService.id ? { ...data, id: s.id } : s));
    } else {
      setServices([...services, { ...data, id: Date.now().toString() }]);
    }
  };

  const handleDelete = (id: string) => {
    if (confirm('确定要删除该技术服务吗？')) {
      setServices(services.filter(s => s.id !== id));
    }
  };

  const getStatusBadge = (status: TechService['status']) => {
    const styles = {
      running: 'bg-emerald-50 text-emerald-600 border-emerald-200',
      stopped: 'bg-slate-100 text-slate-500 border-slate-200',
      error: 'bg-red-50 text-red-600 border-red-200',
    };
    const labels = { running: '运行中', stopped: '已停止', error: '异常' };
    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${styles[status]}`}>
        <span className={`w-1.5 h-1.5 rounded-full ${status === 'running' ? 'bg-emerald-500' : status === 'stopped' ? 'bg-slate-400' : 'bg-red-500'}`}></span>
        {labels[status]}
      </span>
    );
  };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">数据技术服务</h2>
          <p className="text-slate-500 text-sm mt-1">管理和配置数据技术服务</p>
        </div>
        <button
          onClick={handleAdd}
          className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-all text-sm"
        >
          <FontAwesomeIcon icon={faPlus} className="text-xs" />
          新增服务
        </button>
      </div>

      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full">
          <thead className="bg-slate-50">
            <tr>
              <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">服务名称</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">版本</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">状态</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">描述</th>
              <th className="text-right px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">操作</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {services.map(service => (
              <tr key={service.id} className="hover:bg-slate-50">
                <td className="px-6 py-4">
                  <span className="text-slate-800 font-medium">{service.name}</span>
                </td>
                <td className="px-6 py-4">
                  <span className="text-slate-500 text-sm">{service.version}</span>
                </td>
                <td className="px-6 py-4">
                  {getStatusBadge(service.status)}
                </td>
                <td className="px-6 py-4">
                  <span className="text-slate-500 text-sm">{service.description}</span>
                </td>
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-1.5">
                    <button
                      onClick={() => handleEdit(service)}
                      className="p-2 rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-100 transition-all"
                    >
                      <FontAwesomeIcon icon={faEdit} className="text-sm" />
                    </button>
                    <button
                      onClick={() => handleDelete(service.id)}
                      className="p-2 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-all"
                    >
                      <FontAwesomeIcon icon={faTrash} className="text-sm" />
                    </button>
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {services.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500">暂无数据技术服务</p>
          </div>
        )}
      </div>

      <ServiceModal
        isOpen={showModal}
        onClose={() => setShowModal(false)}
        onSave={handleSave}
        editingService={editingService}
      />
    </div>
  );
}
